...
select count(categoria_id) from tb_produtos; /*não conta valores nulos*/

select sum(preco) from tb_produtos;

select avg(preco) as media_preço from tb_produtos;

select  categoria_id, avg(preco) as media_preço from tb_produtos group by categoria_id;
...
select tb_produtos.nome, tb_produtos.preco, tb_produtos.qtproduto, tb_categoria.descricao, tb_categoria.ativo from tb_produtos inner join tb_categoria on tb_categoria.id = tb_produtos.categoria_id;  

select tb_produtos.nome, tb_produtos.preco, tb_produtos.qtproduto, tb_categoria.descricao, tb_categoria.ativo from tb_produtos left join tb_categoria on tb_categoria.id = tb_produtos.categoria_id;  

select tb_produtos.nome, tb_produtos.preco, tb_produtos.qtproduto, tb_categoria.descricao, tb_categoria.ativo from tb_produtos right join tb_categoria on tb_categoria.id = tb_produtos.categoria_id;  